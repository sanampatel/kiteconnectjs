var KiteTicker = function(api_key, user_id, public_token, address) {
	// Defing variables here so less keep code away
	var user_api_key = "YOUR_API_KEY";
	var user_user_id = "YOUR_USER_ID_KEY";
	var user_public_token = "YOUR_PUBLIC_TOKEN_KEY";

	if(!api_key) { var api_key = user_api_key }
	if(!user_id) { var user_id = user_user_id }
	if(!public_token) { var public_token = user_public_token }
	if(!address) { var address = "wss://websocket.kite.trade/"; }
			...
			switch(segment) {
				case NseIndices:
					...
					var q = {
						...
						PriceChangeFromClosingPrice: (buf2long(bin.slice(4,8)) / dec) - (buf2long(bin.slice(20,24)) / dec)
						// Adding PriceChangeFromClosingPrice so less code on frontend!
					};
				...
				case NseCD:
					...
					var q = {
						...
						PriceChangeFromClosingPrice: (buf2long(bin.slice(4,8)) / dec) - (buf2long(bin.slice(40,44)) / dec),
						Depth: {"buy": [], "sell": []}
					};

					// Change % & Change
					q.NetPriceChangeFromClosingPrice = 0;
					q.PriceChangeFromClosingPrice = 0;
					if(q.ClosePrice !== 0) {
						q.NetPriceChangeFromClosingPrice = (q.LastTradedPrice - q.ClosePrice)*100 / q.ClosePrice;
						q.PriceChangeFromClosingPrice = (q.LastTradedPrice - q.ClosePrice)*100;
					}
